...
- 이때 ARP 요청 패킷에는 A의 IP 주소, MAC 주소, 그리고 목적지인 B의 IP 주소를 넣는다. (목적지 MAC 주소는 비워둔다.)

3. **브로드캐스트 전송**
- ARP 패킷을 데이터 링크 계층 프레임에 캡슐화한다.
    - ARP 패킷을 이더넷 프레임에 넣어 전송한다는 뜻이다.
    - 이더넷 프레임의 payload에 ARP 패킷을 넣고 이더넷 프레임의 목적지 MAC 주소를 FF:FF:FF:FF:FF:FF(브로드캐스트)로 설정해서 전송한다는 의미다.
